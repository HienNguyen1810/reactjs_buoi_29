...
import { save } from "../redux/studentSlice";
...
function CreateStudentForm() {
	...
	return (
		<>
			<Header style={{ minHeight: "60px" }} className="bg-dark text-white">
					...
					<Col flex={6}>
						<Form.Item
							...
							rules={[
								...
								() => ({
									validator(_, value) {
										return students.some((d) => d.id === value)
					...
					<Col flex={6}>
						<Form.Item
							...
							rules={[
								...
								() => ({
									validator(_, value) {
										return students.some((d) => d.phoneNumber === value)
					...
					<Col flex={6}>
						<Form.Item
							...
							rules={[
								...
								() => ({
									validator(_, value) {
										return students.some((d) => d.email === value)
